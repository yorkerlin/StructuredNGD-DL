...
def _extract_patches(x, kernel_size, stride, padding, groups, compute_grad=True):
    """
...
class ComputeCovA:
    ...
    def conv2d(a, layer):
        batch_size = a.size(0)
        a = _extract_patches(a, layer.kernel_size, layer.stride, layer.padding, layer.groups, compute_grad=False)
...
class ComputeCovG:
    ...
    def conv2d(g, layer, batch_averaged):
        ...
        g = try_contiguous(g)
        g = g.view(-1, g.size(-1)) #(batch_size*out_h*out_w, n_filters)

        if batch_averaged:
            g = g * batch_size
        g = g * spatial_size

        cov_g = g.t() @ (g / g.size(0)) # (batch_size *batch_size * out_h *out_w)/(batch_size*out_h*out_w) = batch_size
        #batch_size**2 * spatial_size / g.size(0) == batch_size
